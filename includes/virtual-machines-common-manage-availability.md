...
> [!IMPORTANT] Two or more Virtual machines are required in an Availability Set to avoid downtime during Azure Planned Maintenance events. Single instance VM's may qualify for the [self-service proactive redeploy](../articles/virtual-machines/windows/maintenance-notifications.md). 
It is not best practice for a Single instance VM to be part of an Availability Set but a VM deployed this way will still be eligible to receive Planned Maintenance notifications and would still be considered for the self-service proactive redeploy. 
The Azure SLA applies for single instance VMs using [Azure Premium Storage](../articles/virtual-machines/windows/premium-storage.md).
